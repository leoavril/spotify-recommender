...
def processPlaylistForClustering(playlists, tracks):
    ...
    playlistSongSparse = dok_matrix(
        (len(playlistIDs), len(trackIDs)), dtype=np.float32)
...
def list_blobs_hierarchical(container_client, prefix="data/"):
    ...
    for blob in container_client.walk_blobs(name_starts_with=prefix, delimiter='/'):
        if isinstance(blob, BlobPrefix):
            files.extend(list_blobs_hierarchical(
...
def createDFs(idx, numFiles, blob_service_client):
    ...
    files = [blob_service_client.get_blob_client(
    ...
    tracksDF["tid"] = tracksDF.apply(
        lambda row: parseTrackURI(row["track_uri"]), axis=1)
    ...
    tracksDF["sparse_id"] = tracksDF.apply(
        lambda row: IDtoIDXMap[row["tid"]], axis=1)
    ...
    blob_service_client.get_blob_client(
        "data", "lib/playlists.pkl").upload_blob(playlist_pickle, overwrite=True)
    ...
    blob_service_client.get_blob_client(
        "data", "lib/tracks.pkl").upload_blob(tracks_pickle, overwrite=True)
    ...
    blob_service_client.get_blob_client(
        "data", "lib/playlistSparse.pkl").upload_blob(playlist_clustered_pickle, overwrite=True)
